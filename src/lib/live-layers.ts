...
export const LIVE_LAYERS: LiveLayerSpec[] = [
  ...
  {
    id: 'deepstate-frontline',
    label: 'layers.frontline',
    kind: 'feed',
    renderer: 'both',
    ...
    attribution: {
      ...
      notice: 'Frontline data © DeepStateMAP. Shown with the map date they publish; not an official source.',
    },
    scope: ['ukraine-war'],
    requiresKey: false,
    transport: 'http',
  },
  {
    ...
    label: 'layers.pendingCandidates',
    kind: 'feed',
    renderer: 'globe-home',
    url: '/_hourly/alerts.json',
    ttlMs: 5 * MINUTE,
    cors: 'same-origin',
    attribution: {
      source: 'Watchboard light scan',
      license: 'CC BY 4.0',
      url: 'https://watchboard.dev/breaking-news-audit/',
      notice: 'Geolocated light-scan candidates that have not passed triage yet. Unverified by definition.',
    },
    scope: [],
    requiresKey: false,
    transport: 'http',
  },
  {
    ...
    attribution: {
      ...
      notice: 'Orange and Red alerts of the last 7 days, ingested by the light scan every 15 minutes.',
    },
    scope: [],
    requiresKey: false,
    transport: 'http',
