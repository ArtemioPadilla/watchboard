...
| BL-020 | API health indicator | ✅ Done 2026-09-07 (plan E2.H4): `SourceStatusChip` + degraded-sources summary on the globe HUD/controls and the 2D layer panel, fed by `src/lib/live-source.ts` statuses (ok / stale / rate-limited / error / snapshot date) | S | Dev + PM: shipped with the live-source cache |
...
| BL-024 | KPI trend deltas | Add optional `delta` field to `KpiSchema` showing change direction/magnitude on each KPI card | S | Done (E7): `deltaDetail {value, direction, period}` in `KpiSchema`, rendered by `KpiStrip.astro`, filled by the nightly updater, feeds the activity index |
