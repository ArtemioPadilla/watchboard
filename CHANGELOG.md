...
### Fixed — 2026-04-28
- **Light-scan high-confidence breaking news now reaches tracker data** (CJNG El Jardinero detention regression): `scripts/hourly-light-scan.ts` was routing scores ≥ 0.85 only to Telegram + audit log, never queuing for the heavy scan. The Marina detention of "El Jardinero" (CJNG successor) was detected twice at 0.87 on 2026-04-27 but `mencho-cjng/data/events/*.json` was never updated because the heavy scan (which is what writes tracker JSON) never saw the candidate. High-score hits now also append to `pending-candidates.json` so the next 6-h heavy scan promotes them to AI triage. Telegram remains the realtime alert channel; the queue is what closes the data-update loop.
- **Heavy scan was silently dropping every pending candidate**: `scripts/hourly-scan.ts` deduped pending entries against `seenUrls` (built from `state.seen` + the in-flight RSS batch). By the time the heavy scan runs, every pending candidate's URL is already represented in `state.seen` (the light scan that queued it persisted the URL there in the same run), so the dedup filtered every pending entry out before AI triage — the moderate-score path effectively never reached triage either. Pending entries are explicitly queued items that already passed dedup once; they now bypass the `state.seen` check and dedup only against the current batch. The promotion logic is extracted into an exported `promotePendingCandidates()` helper covered by unit tests, and `[hourly-scan] promoted N pending candidate(s) to triage` is logged so the path is observable in workflow output.
