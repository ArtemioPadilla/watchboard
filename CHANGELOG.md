...
- **Heavy scan was silently dropping every pending candidate**: `scripts/hourly-scan.ts` deduped pending entries against `seenUrls` (built from `state.seen` + the in-flight RSS batch). The light scan always pushes a candidate's URL to `state.seen` *before* queuing it, so every pending entry was guaranteed to be in `seenUrls` and was filtered out — the moderate-score path also never reached AI triage. Pending entries are explicitly queued items that have already passed dedup once; they now bypass the `state.seen` check and dedup only against the current batch. Logs `[hourly-scan] promoted N pending candidate(s) to triage` so this is observable in workflow output.
