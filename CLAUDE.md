...
- `alerts.json` entries gain `resolved: true` once a tracker event from the last 7 days cites the same URL (`scripts/lib/event-urls-node.ts`). The homepage globe (`GlobePanel.tsx`, `htmlElementsData`) shows unresolved geolocated alerts as dotted tier-4 `.cc-pending-pin` elements titled "Unverified candidate · source · place" (registry layer `pending-candidates`); a resolved one drops out so the real event takes its place.
